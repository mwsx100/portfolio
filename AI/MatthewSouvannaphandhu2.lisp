...
(defun sigmoid (u)                ;Sigmoid function   
    (/ 1 (+ 1 (expt (exp 1)(* -1 u)))))


(defun net-error (output correct-output)  ;net error function, takes output and correct-output vectors, returns a scalar value
   (e (scalar-multiply 0.5 (multiply (transpose(subtract correct-output output))   (subtract correct-output output))) 1 1 ))

(defun forward-propagate (datum v w) ;forward propagation
    (map-m #'sigmoid (multiply w (map-m #'sigmoid (multiply v (car datum)))))) ;I hope this is all I had to do here
...
(defun back-propagate (alpha datum v w)
    (let* ((h ( map-m #'sigmoid (multiply v (car datum)))) ;initialize h
     (o (map-m #'sigmoid (multiply w h))) ;initialize o
          (o-delta (e-multiply (subtract (cadr datum) o) o (subtract-from-scalar 1 o))) ;initialize o-delta
          (h-delta (e-multiply h (subtract-from-scalar 1 h) (multiply (transpose w) o-delta))) ;initialize h-delta
          (w (add w (scalar-multiply alpha (multiply o-delta (transpose h))))) ;new value for w
          (v (add v (scalar-multiply alpha (multiply h-delta (transpose (car datum)))))) ) ;new value for v
          (list v w)) ) ;new v and w put into a list
...
(defun mean (scalar-list) ;I didn't see the average function provided when I wrote this but this seems to work fine
    (/ (apply '+ scalar-list) (length scalar-list))) ;returns the average of a scalar list

(defun net-build (data num-hidden-units alpha initial-bounds max-iterations modulo &optional print-all-errors)
    (let* ((data (shuffle data)) (m 0) ;shuffles data and initializes the counter to let us know when we are at the modulo iteration
                                 (vw (list (make-random-matrix num-hidden-units (length (caar data)) initial-bounds)  ;initializes list of vectors, vw
                                           (make-random-matrix (length (cadar data)) num-hidden-units initial-bounds))) ) 
          (dotimes (n max-iterations)
              (setf data (shuffle data)) (setf m (+ 1 m)) (when (= m modulo)  ;increments m and does forward propagation when m = modulo
				 (let ((err (list))) ;initializes a list of net error for each element
                                   (dolist (x data) (print (cadr x)) (print (forward-propagate x (car vw) (cadr vw)))
                                      (setf err (cons (net-error (forward-propagate x (car vw) (cadr vw)) (cadr x)) err)) ;adds net error of current element to err list
                                       (if (equalp nil (equalp nil print-all-errors)) (print (car err)))) ;prints every error if print-all-errors is true 
				   (if (= modulo max-iterations) (return-from net-build vw)) ;if we are in simple-generalization, don't print anything
                                   (let ((worst-e (loop for y in err maximize y))) 
                                  (print "Worst Error:")  (print worst-e) (print "Mean Error:") (print (mean err)) (setf m 0) ;prints the max error and mean error at the end of the loop, sets m back to 0
                                        (if (< worst-e *a-good-minimum-error*) (return-from net-build vw))))) ;returns from function if the worst error is lower than the minimum error
              (dolist (x data)
                  (setf vw (back-propagate alpha x  (car vw) (cadr vw))) )) ;(print "hi") ;does backpropogation and updates vw's values                     
              vw ));returns vw here
...
(defun round-to (num prec) ;rounds to the specified precision. I thought this would help in generalization but I could be wrong
	   (let ((div (expt 10 prec)))
	    (float (/ (funcall #'round (* num div)) div))) )

(defun even (num) ;if a number is odd, make it even. used to find the middle half of a list of data
  (if (= 0 (mod num 2)) num
	 (+ num 1)))

(defun simple-generalization (data num-hidden-units alpha initial-bounds max-iterations)
  (let* ( (h1 (subseq data 0 (/ (even (length data)) 2))) ;cuts data in half, h1 is the first half
	 (h2 (subseq data (/ (even (length data)) 2) (length data))) ;h2 is the second half
	 (vw (net-build (convert-data h1) num-hidden-units alpha initial-bounds max-iterations max-iterations)) ) ;initializes vw by calling net-build
    	 (let ((err (list))) ;initializes a list of net error for each element
	   (dolist (x (shuffle (convert-data h2)))  ;shuffles data, not sure if i needed to do this    ;(print (cadr x)) (print (round-to (caar (forward-propagate x (car vw) (cadr vw))) 1))
           (setf err (cons (net-error (list (list (round-to (caar (forward-propagate x (car vw) (cadr vw))) 1))) (cadr x)) err))) ;rounds the result of forward-propagate to the nearest 10th and calls net-error to 
	   (mean err) ) )) ;returns mean error                                                                                                       ;compare with expected output
...
;WRITE UP
;Finishing up the code here was easier for me than the first project. Its probably because I am getting more used to lisp. The helper functions provided helped out a lot.
; I would like to have had more time to work on this so I could have made my neural network a bit smarter but it seems adequate as it is right now. I probably spent the most 
;time on net-build which makes sense since its the function where the neural network is built. After I finished net-build I tested it on the nand and xor data. It seems to 
;learn NAND and XOR more efficiently if I set alpha to 1.5. When I had the alpha at 1.0, it would get exit the function before 20000 iterations at most 60 percent of the time. 
;When I raised it to 1.5, it would exit before 20000 iterations around 75 percent of the time. This showed me how the optimal parameters for net-build vary depending on the
; problem. On simple-generalization, I decided to round the results of forward-propagation to the nearest tenth to see if it reduces the average error by any amount. It gave me
; more cases where there was zero error but it did not really reduce the average error overall. The average error that got returned by simple generalization when tested on voting 
;records was about .02, which is not as small as I would like it to be but I have not figured out any tricks to making this number a bit better. Getting to this point in the project 
;helped me better understand the concept of neural networks, but I feel like I have a lot more to learn. When I was running net-build on the NAND data, sometimes when it was unable to
; get to the exit condition, I saw that the mean data and worst data started to print out values that were around the same as the last values printed. I believe this means that the 
;data has converged on a local minima and that is why the learning seems to have plateaued. I was not able to implement anything that could have fixed this problem but adjusting the 
;alpha seemed to make this problem happen a lot less. In the future, I might try adding in a function that optimizes the alpha depending on the problem and adjusts it as the network 
;is being built. I am not sure how I would go about adjusting it though. This is mainly for situations where it converges on a local minima, but I am not sure how figure out whether or 
;not I am stuck at a local minima. I also wanted to mention that this program takes the longest to run out of anything else I have written. When I was reading up on neural networks I 
;found that many sources say that this is typical. In any case, my neural network can be fed data and classify it faster than I could, so I can see how this can be very useful.
