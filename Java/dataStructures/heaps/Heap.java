...
public class Heap<T extends Comparable<T>>{
	...
	public void enqueue(T data) {
        ...
        while (true) {
        	if (datum[n].compareTo(datum[getParent(n)]) > 0) {
        		datum[n] = datum[getParent(n)];
	...
	public T dequeue() {
		...
		if (datum[getLeft(n)] == null && datum[getRight(n)] != null) {
			if (datum[getRight(n)].compareTo(datum[n]) > 0) {
			T data0 = datum[n];
			datum[n] = datum[getRight(n)] ; 
			datum[getRight(n)] = data0;
			n = getRight(n); }
			return rData; }
