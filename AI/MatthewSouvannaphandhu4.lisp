...
(defun num-out (state)
    (let ( (diff 0)  (goal (vector 1 2 3 4 5 6 7 8 9 8))(puzzle (puzzle-from-state state))  )   
    (loop for x across puzzle for y across goal
          when (equalp (equalp x y) nil)
           do (setq diff (+ 1 diff)))
    (if (>= diff 2) (+ (- diff 2) (depth state)) (+ diff (depth state)))))   

(defparameter *goal-vals* '((0 0) (1 0) (2 0) (0 1) (1 1) (2 1) (0 2) (1 2) (2 2)))

(defun manh (state)
    (let ( (x 0) (y 0)(puzzle (puzzle-from-state state)) )
    (loop for n from 0 to 8 do
	 (if (/= (elt puzzle n) 9) (progn 
          (setf x (+ x (abs (- (car (elt *goal-vals* n)) (car (elt *goal-vals* (- (elt puzzle n) 1)))))))
          (setf y (+ y (abs (- (cadr (elt *goal-vals* n)) (cadr (elt *goal-vals* (- (elt puzzle n) 1))))))))))
         (+ x y) ))
       
(defun process (c func)
 ;   (if (equalp func 'dfs-enqueuer) )
  ;  (if (equalp func 'bfs-enqueuer) )
    (if (equalp func 'manhattan-enqueuer)
        (return-from process (+ (manh c) (depth c))))
    (if (equalp func 'num-out-enqueuer)
	(progn; (print c) (print "in process")
        (return-from process (+ (num-out c) (depth c)))))
    (depth c))

(defun general-search (initial-state goal-test enqueueing-function &optional (maximum-iterations nil))
	   (let* ((q (make-empty-queue)) (hist (make-hash-table :test 'equalp)) (s initial-state)
		 (iter 0))
	     (funcall enqueueing-function s q); (print q) (print "queueing1") (print (puzzle-from-state s))
	     (setf (gethash (puzzle-from-state s) hist) s) (setf s (remove-front q))	   
             (loop
                    (setf iter (+ iter 1))
                    (if (> iter maximum-iterations) (return-from general-search 'failed) )	    
                    (setf s (remove-front q)) ;(print (car s)) (print "hi")
                    (if (goal-p (car s)) (progn (print iter) (return-from general-search (print-solution s))))
		    (foreach-valid-move (move (puzzle-from-state s)) ;(print (make-move move (car s)))
                            (let* ( (c (build-state (make-move move (car s)) (cdr s))) (w (process c enqueueing-function))) ;(print (gethash (puzzle-from-state c) hist)) (print (cdr s)) (print "xxx") (print  (cadr c))
                                 (if (not (equalp nil (gethash (puzzle-from-state c) hist)))
                                     (if (< w (process (gethash (puzzle-from-state c) hist) enqueueing-function)) (progn; (print (gethash (puzzle-from-state c) hist)) (print w)
                                         (setf (gethash (puzzle-from-state c) hist) (cdr c))))
                                     (progn  (funcall enqueueing-function c q); (print (not (equalp nil (gethash (puzzle-from-state c) hist))))
                                         (setf (gethash (puzzle-from-state c) hist) (cdr c)) )))))))
...
(defun goal-p (state)
    (if (equalp state '(#(1 2 3 4 5 6 7 8 9 8))) t
        nil))
...
(defun dfs-enqueuer (state queue)
  (enqueue-at-front queue state)
    (foreach-valid-move (move (puzzle-from-state state)) 
        (enqueue-at-front queue (build-state (make-move move (puzzle-from-state state)) state))))
...
(defun bfs-enqueuer (state queue)
    (enqueue-at-end queue state)
      (foreach-valid-move (move (puzzle-from-state state)) 
        (enqueue-at-end queue (build-state (make-move move (puzzle-from-state state)) state))))
...
(defun manhattan-enqueuer (state queue)
   (enqueue-by-priority queue 'manh state)
     (foreach-valid-move (move (puzzle-from-state state))
         (enqueue-by-priority queue 'manh (build-state (make-move move (puzzle-from-state state)) state))))
...
(defun num-out-enqueuer (state queue)
 (enqueue-by-priority queue 'num-out state)
      (foreach-valid-move (move (puzzle-from-state state))
         (enqueue-by-priority queue 'num-out (build-state (make-move move (puzzle-from-state state)) state))))
...
|#
; In this project I learned the magnitude in which some types of algorithms can be really unfit for certain tasks
; as well as renwed my appreciation for heaps. The heuristic functions manhattan and num-out are literally exponentially
; better at finding solutions to the 8 puzzle. I was only able to get to the second problem state given and find a solution
; that was found in the specified amount of turns or less. The puzzle I was supposed to solve in 16 moves I was only able to get down to 23 moves using the
; manhattan function. A big problem I had in implementing this was that I kept accidentally adding duplicates to the queue even when I was checking  
;with the history list. I had to go line by line and print out the states being compared at that point. I had no idea why there were duplicates being added
;I realized that when I first started enqueing, I forgot to enqueue the root node, so it would add itself twice because it thought it had not been added yet.
;
;
;
;
